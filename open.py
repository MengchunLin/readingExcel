...
#             row_index_N, col_index_N = np.where(df == 'N')
#             row_index_E, col_index_E = np.where(df == 'E')
#             col_index_N+=1
#             col_index_E+=1

#             N_2 = df.iloc[row_index_N,col_index_N]
#             N_2_value=N_2.values

#             E_2=df.iloc[row_index_E,col_index_E]
#             E_2_value=E_2.values
            
#             length=pow(pow(N_2_value-N_1_value,2)+pow(E_2_value-E_1_value,2),0.5)
#             sum=sum+length
#             N_1_value=N_2_value
#             E_1_value=E_2_value

# element=10*scale_factor_w/sum
#print(element)
# text="type"
# insert=APoint(0,0)
# height=0
# textObj=acad.model.AddText(text,insert,height)
for index, sheet_name in enumerate(sheet_names):
    #skip the first sheet
    if index != 0:
        start_point = 0
...
#------------------------------------------------------------------------------------------------------------------------------------
        ...
        row_index_N, col_index_N = np.where(df == 'N')
        row_index_E, col_index_E = np.where(df == 'E')
        row_index_EL,col_index_EL=np.where(df=='Ground EL')
        col_index_N+=1
        col_index_E+=1
        col_index_EL+=1
        N = df.iloc[row_index_N,col_index_N]
        N_value=N.values
        E=df.iloc[row_index_E,col_index_E]
        E_value=E.values
        distance=pow(N_value*N_value+E_value*E_value,0.5)
        EL=df.iloc[row_index_EL,col_index_EL]
        EL_value=EL.values
 ...
 #------------------------------------------------------------------------------------------------------------------------------------
        ...
        text_value='Depth(m)'
        # AlignNum=0
        # textObj.Aligment=AlignNum
        text=acad.AddText(text_value,APoint(index * scale_factor_w*distance-10*scale_factor_w, EL_value*scale_factor_h),1*scale_factor_w)
 #------------------------------------------------------------------------------------------------------------------------------------
        #spt
        text_value='SPT-N'
        text=acad.AddText(text_value,APoint(index * scale_factor_w*distance+5*scale_factor_w, 2.5*scale_factor_h),1*scale_factor_w)
 ...
 #------------------------------------------------------------------------------------------------------------------------------------       
        ...
        if not pd.isna(GWL):
            GWL_point=APoint(index*distance*scale_factor_w-(10*scale_factor_w),-GWL*scale_factor_h)
            #水位線
            GWL_point_end=APoint(index*distance*scale_factor_w-(10*scale_factor_w)-(3*scale_factor_w),-GWL_point*scale_factor_h)
            acad.AddLine(GWL_point,GWL_point_end)
            #裝飾線
            acad.AddLine(APoint(index*distance*scale_factor_w-(10*scale_factor_w)-(0.8*scale_factor_w),-GWL*scale_factor_h-0.2*scale_factor_h),
                        APoint(index*distance*scale_factor_w-(10*scale_factor_w)-(2.2*scale_factor_w),-GWL*scale_factor_h-0.2*scale_factor_h))
            acad.AddLine(APoint(index*distance*scale_factor_w-(10*scale_factor_w)-(1*scale_factor_w),-GWL*scale_factor_h-0.4*scale_factor_h),
                        APoint(index*distance*scale_factor_w-(10*scale_factor_w)-(2*scale_factor_w),-GWL*scale_factor_h-0.4*scale_factor_h))
            #箭頭
 ...
 #------------------------------------------------------------------------------------------------------------------------------- ----- 
        ...
        for layer_index, row in df.iterrows():
            ...
            if layer_index != 0:
                ...
                previous_layer = EL.values-depth
                pnts=[p1.x,p1.y,
                       p2.x,p2.y,   
                       p4.x,p4.y,
                       p3.x,p3.y,
                ...
                if not pd.isna(Layer):
                    ...
                    text = acad.AddText(Layer_text, APoint(index * distance*scale_factor_w - 5 * scale_factor_w, -Layer * scale_factor_h), 0.8 * scale_factor_w)
                                
                nan_encountered = False
                #spt_n
                if not pd.isna(spt_n):
                    spt_text = spt_n
                    text = acad.AddText(spt_text, APoint(index * distance*scale_factor_w + 7 * scale_factor_w, -depth * scale_factor_h), 0.8 * scale_factor_w)
                else:
